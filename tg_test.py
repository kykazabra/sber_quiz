...
def start(message):
    ...
    if not user_data:
        ...
        db.answer.insert_one(
            {"user_id": chat_id, "type": 'start_1', 'time': datetime.datetime.now().strftime('%H:%M:%S')})
...
def handle_direction(message):
    ...
    q_type = message.text
    db.answer.insert_one(
        {"user_id": chat_id, 'type': f"{q_type}_level_1", 'time': datetime.datetime.now().strftime('%H:%M:%S')})
